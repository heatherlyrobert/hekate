...
YVIKEYS__prep_eprint    (tPROC *a_proc)
{
   ...
   DEBUG_GRAF  yLOG_complex ("eprint"    , "Â%sÊ", a_proc->e_print);
   strlcpy (a_proc->m_print, x_mprint, LEN_HUND);
   DEBUG_GRAF  yLOG_complex ("mprint"    , "Â%sÊ", a_proc->m_print);
   /*---(update)-------------------------*/
   ++(x_exec->e_seq);
   /*---(complete)-----------------------*/
   DEBUG_GRAF  yLOG_exit    (__FUNCTION__);
...
YVIKEYS__prep_mprint    (tPROC *a_proc)
{
   ...
   for (i = 1; i <= my.e_mult; ++i) {
      if (my.e_flag [i] > 0) {
         if (x_exec->e_seq == 1)  x_mprint [(i * 2) - 2] = 'å';
         else                     x_mprint [(i * 2) - 2] = 'Å';
      }
   }
   ...
   if (x_exec->e_col > 0) {
      for (i = x_exec->e_col; i <= my.e_mult; ++i) {
         x_mprint [i * 2 - 1] = 'Ä';
         if (i > x_exec->e_col) {
            if (my.e_flag [i] > 0)  x_mprint [i * 2 - 2] = 'å';
            else                    x_mprint [i * 2 - 2] = 'Ä';
         }
      }
   }
   /*---(save)---------------------------*/
...
YVIKEYS__prep_pprint    (tPROC *a_proc)
{
   ...
   snprintf (x_tprint, TLEN, "ÄÄ%dÄÄ-", a_proc->t_count);
   /*---(save)---------------------------*/
...
YVIKEYS__prep_lprint    (void)
{
   ...
   while (x_libs != NULL) {
      ...
      case LIB_EVERY : case LIB_CORES : case LIB_MAJOR : case LIB_HEATH :
         ...
         snprintf (x_cnt2, 5, "Ä%dÄÄÄ", x_libs->e_count);
         break;
      case LIB_OTHER :
         snprintf (x_cnt , 6, "œÄ%dÄÄ", x_libs->t_count);
         snprintf (x_cnt2, 5, "ÄÄÄÄÄÄ");
         break;
      }
...
YVIKEYS__exec_show      (tPROC *a_proc, int a_wide)
{
   ...
   if (x_eline > 0) {
      ...
      DEBUG_GRAF  yLOG_complex ("eprint"    , "Â%sÊ", a_proc->e_print);
      DEBUG_GRAF  yLOG_complex ("mprint"    , "Â%sÊ", a_proc->m_print);
      snprintf (t, a_wide, "%s%sœ", a_proc->e_print, a_proc->m_print);
      DEBUG_GRAF  yLOG_complex ("normal"    , "Â%sÊ", t);
   ...
   if (x_eline == x_pline) {
      yCOLOR_curs ("9_form");
      snprintf (t, a_wide, "%s%sœ", x_exec->e_print, a_proc->m_print);
      ...
      for (i = 0; i < x_len; ++i) {
         ...
         case ' ' :
            mvprintw (x_eline, my.e_wide + i, "Ä");
            break;
         case 'Å' : case 'å' :
            mvprintw (x_eline, my.e_wide + i, "å");
            break;
         case 'á' :
            mvprintw (x_eline, my.e_wide + i, "ä");
            x_turn = i;
            break;
         case 'Ñ' :
            mvprintw (x_eline, my.e_wide + i, "à");
            x_turn = i;
            break;
         default  :
            x_turn = i;
...
YVIKEYS__proc_show      (tPROC *a_proc, int a_wide)
{
   ...
   snprintf (t, x_wide, "%s∑%sœ", a_proc->p_print, a_proc->t_print);
...
YVIKEYS__main_exec      (int a_wide)
{
   ...
   if (x_wide > l - 1)  mvprintw (x_link, l - 1, "œ");
   x_len = strlen (x_proc->m_print);
   for (i = 0; i < x_len; ++i) {
      switch (x_proc->m_print [i]) {
      case 'á' : case 'Ñ' :
         x_turn = i;
      case 'œ' : case 'Ä' : case 'É' : case 'â' :
   ...
   for (i = 0; i <= x_turn; ++i) {
      ...
      case ' ' :
         if (x_wide >= my.e_wide + i)  mvprintw (x_line, my.e_wide + i, "Ä");
         break;
      case 'á' : case 'Ñ' :
      case 'œ' : case 'Ä' : case 'É' : case 'â' :
...
YVIKEYS__main_libbase (char a_group, int a_left, int a_wide, int a_tall)
{
   ...
   while (x_libs != NULL) {
      if (x_libs->u_flag == a_group) {
         ...
         /*---(connector)----------------*/
         yCOLOR_curs ("9_dang");
         if (x_vert <= a_tall)  mvprintw (x_vert, a_left - 2, "å ");
         attrset (0);
         /*---(done)---------------------*/
      }
      LIBS_by_cursor (&x_libs, YDLST_DNEXT);
   ...
   while (x_ties != NULL) {
      ...
      if (x_libs->u_flag == a_group) {
         ...
         if (x_libs->l_seq <= a_tall) {
            switch (x_libs->u_flag) {
            case LIB_EVERY :
               if (x_libs->l_seq == 2)     mvprintw (x_libs->l_seq, a_left - 2, "ÉÄ%s", t);
               else                        mvprintw (x_libs->l_seq, a_left - 2, "áÄ%s", t);
               ++(*c);
               break;
            case LIB_CORES :
               if (x_libs->l_seq == a_mid)  *a_flag = 'y';
               mvprintw (x_libs->l_seq, a_left - 2, "áÄ%s", t);
               ++(*c);
               break;
            case LIB_MAJOR : case LIB_OTHER :
               if (x_libs->l_seq < a_max)  mvprintw (x_libs->l_seq, a_left - 2, "áÄ%s", t);
               else                        mvprintw (x_libs->l_seq, a_left - 2, "ÑÄ%s", t);
               ++(*c);
               break;
            case LIB_HEATH :
               attrset (0);
...
YVIKEYS__main_libproc (int a_line, int a_left, int a_mid, int a_conn, char a_flag, int a_wide, int a_tall)
{
   ...
   yCOLOR_curs ("9_dang");
   /*---(lib connector)------------------*/
   if (a_flag == 'y')  mvprintw (a_mid, a_left - 2, "ä");
   else                mvprintw (a_mid, a_left - 2, "Ü");
   if (a_line <  a_mid) {
      mvprintw (a_line, a_conn, "ÄÇ");
      mvprintw (a_mid , a_conn + 1, "ÑÄ");
      for (i = a_line + 1; i < a_mid; ++i)  mvprintw (i , a_conn + 1, "Å");
   } else if (a_line == a_mid) {
      mvprintw (a_line, a_conn, "ÄÄÄ");
   } else if (a_line >  a_mid) {
      mvprintw (a_line, a_conn, "ÄÖ");
      mvprintw (a_mid , a_conn + 1, "ÉÄ");
      for (i = a_mid + 1; i < a_line; ++i)  mvprintw (i , a_conn + 1, "Å");
   }
   /*---(unset color)--------------------*/
   attrset (0);
   /*---(complete)-----------------------*/
...
YVIKEYS__main_libtitle  (int a_left, int e, int c, int o, int a_wide, int a_tall)
{
   ...
   attrset (0);
   yCOLOR_curs ("9_dang");
   if (x_vert <= a_tall)  mvprintw (x_vert - 1, a_left - 2, "å");
   if (x_vert <= a_tall)  mvprintw (x_vert    , a_left - 2, "å");
   attrset (0);
   ...
   yCOLOR_curs ("9_dang");
   if (x_vert <= a_tall) {
      mvprintw (x_vert - 1, a_left - 2, "å");
      if (o > 0)    mvprintw (x_vert    , a_left - 2, "å");
      else          mvprintw (x_vert    , a_left - 2, "œ");
   }
   attrset (0);
   /*---(complete)-----------------------*/
   return 0;
...
YVIKEYS__memory_exec    (tEXEC *a_exec, int a_left, int *a_epss, int a_wide, int a_tall)
{
   ...
   /*---(background)---------------------*/
   n = 5;
   IF_FITS  mvprintw ( n, a_left,      "É≤≤≤EXEC≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤Ç");
   for (i =  7; i < 30; ++i) {
      IF_FITS  mvprintw (n, a_left,    "å                           å");
   }
   IF_FITS  mvprintw (n, a_left,      "Ñ≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤Ö");
   /*---(data)---------------------------*/
   ...
   ++n;
   IF_FITS  mvprintw (n, a_left - 2, "á≤≤≤/bin/size≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤Ü");
...
YVIKEYS__memory_proc    (tPROC *a_proc, int a_left, int a_epss, int a_lpss, int a_wide, int a_tall)
{
   ...
   yCOLOR_curs ("9_norm");
   n = 5;
   IF_FITS  mvprintw (n, a_left,      "É≤≤≤PROC≤≤≤≤≤≤≤≤≤≤Ç");
   for (i =  7; i < 30; ++i) {
      IF_FITS  mvprintw (n, a_left,    "å                 å");
   }
   IF_FITS  mvprintw (n, a_left,      "Ñ≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤Ö");
   ...
   ++n;
   IF_FITS  mvprintw (n, a_left - 2, "á≤≤≤/proc/smaps≤≤≤Ü");
   ...
   ++n;
   IF_FITS  mvprintw (n, a_left - 2, "á≤≤≤full≤totals≤≤≤Ü");
   ...
   ++n;
   IF_FITS  mvprintw (n, a_left - 2, "á≤≤≤libraries≤≤≤≤≤Ü");
   ...
   while (x_ties != NULL) {
      ...
      if (x_libs->u_flag == a_group) {
         if (*n % 5 == 0) {
            yCOLOR_curs ("9_norm");
            ++(*y);
            if (*n > 1 && *n < a_tall) mvprintw (*y, a_left - 2,      "á≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤ä≤≤≤≤≤≤≤ä≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤ä≤≤≤≤≤≤≤≤≤≤≤≤≤Üá≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤Ü");
            attrset (0);
         }
         YVIKEYS__libcolor (a_group);
         ++(*n);
...
YVIKEYS__memory_libs    (tPROC *a_proc, int a_left, int *a_lpss, int a_wide, int a_tall)
{
   ...
   yCOLOR_curs ("9_norm");
   n = 5;
   IF_FITS  mvprintw (n, a_left,           "É≤≤≤LIBS≤≤≤≤≤≤≤≤≤≤≤≤≤≤proc≤â≤≤tie≤≤â≤≤≤full≤≤≤≤text≤≤≤cons≤≤≤privâ≤≤≤pss≤≤≤+tieÇÉ≤≤text≤≤≤cons≤≤≤≤bssÇ");
   for (i =  0; i <  y + 2; ++i) {
      IF_FITS  mvprintw (n, a_left,         "å                          å       å                             å             åå                    å");
   }
   IF_FITS  mvprintw (n, a_left,   "Ñ≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤à≤≤≤≤≤≤≤à≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤à≤≤≤≤≤≤≤≤≤≤≤≤≤ÖÑ≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤≤Ö");
   attrset (0);
   /*---(data)---------------------------*/
   a_left += 2;  /* 18 characters */
   y       =  5;
